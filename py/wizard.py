...
# @Last Modified time: 2020-06-19 23:30:07
...
class Wizard:
    ...
    def setup_debug(self):
        if params["debug"]:
            ...
            d.setup_debug(True, file=debug_file, expand_file_opt=params["debug_expand_file_opt"])
            return debug_file
        else:
    ...
    def get_fate(self):
        ...
        while True:
            ...
            if code == "help":
                d.msgbox(
                    "You asked for help concerning the item identified by " "tag {0!r}.".format(tag),
                    height=8,
                    width=40,
                )
            else:
...
You have all my support, be brave!""",
                ...
                assert False, "Unexpected reply from WizardDialog.yes_no_help(): " + repr(reply)
...
def process_command_line():
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "ftE", ["debug", "debug-file=", "debug-expand-file-opt", "help", "version",],
        )
    ...
    for option, value in opts:
        if option == "--debug":
            ...
            assert False, "Unexpected option received from the " "getopt module: '%s'" % option
...
def main():
    ...
    try:
        ...
        print(
            "Error (see above for a traceback):\n\n{0}".format(exc_instance), file=sys.stderr,
        )
        sys.exit(1)
    sys.exit(0)
